...
var chat_overlay__ref: ChatOverlay

var chat_node__ref: ChatNodeDialogResource
...
func applyLanguageOption():
    var line_skipped := self.label__ref.lines_skipped
    match language_manager.current_language:
        LanguageManager.LanguageOption.ja:
            self.text = self.chat_node__ref.tr__ja.text
        LanguageManager.LanguageOption.en:
            self.text = self.chat_node__ref.tr__en.text
        LanguageManager.LanguageOption.zh:
            self.text = self.chat_node__ref.tr__zh.text
        LanguageManager.LanguageOption.ja_original:
            self.text = self.chat_node__ref.tr__ja_original.text
    # Need to set it again.
    self.label__ref.lines_skipped = line_skipped
    self.adjustVisibilityOfDownArrow()
